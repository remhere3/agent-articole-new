...
**URL de baza:** `http://localhost:8002`  
**Swagger interactiv:** `http://localhost:8002/docs`  
**Interfata HTML:** `http://localhost:8002/`  
...
```bash
curl -s http://localhost:8002/api/users | python3 -m json.tool
```
```python
import httpx
r = httpx.get("http://localhost:8002/api/users")
for u in r.json():
...
```bash
curl -s -X POST http://localhost:8002/api/users \
  -H "Content-Type: application/json" \
  -d '{"name": "Ion Popescu", "email": "ion@example.com", "active": true}' \
  | python3 -m json.tool
```
```python
r = httpx.post("http://localhost:8002/api/users", json={
    "name": "Ion Popescu",
...
```bash
curl -s http://localhost:8002/api/users/1 | python3 -m json.tool
```
```python
r = httpx.get("http://localhost:8002/api/users/1")
print(r.json())
```
...
```bash
curl -s -X PUT http://localhost:8002/api/users/1 \
  -H "Content-Type: application/json" \
  -d '{"name": "Ion Popescu Jr.", "active": false}'
```
```python
r = httpx.put("http://localhost:8002/api/users/1", json={
    "name": "Ion Popescu Jr.",
    "active": False
})
print(r.json())
...
```bash
curl -s -X DELETE http://localhost:8002/api/users/1
```
```python
r = httpx.delete("http://localhost:8002/api/users/1")
...
```bash
curl -s http://localhost:8002/api/topics | python3 -m json.tool
```
```python
r = httpx.get("http://localhost:8002/api/topics")
for t in r.json():
...
```bash
curl -s -X POST http://localhost:8002/api/topics \
  -H "Content-Type: application/json" \
  -d '{
    "name": "Terapie Genica",
...
```
```python
r = httpx.post("http://localhost:8002/api/topics", json={
    "name": "Terapie Genica",
    "user_question": "Da-mi toate articolele publicate in reviste de specialitate despre terapia genica si CRISPR",
    "keywords": "gene therapy, CRISPR",
    "days_back": 7,
...
```bash
curl -s http://localhost:8002/api/topics/1 | python3 -m json.tool
```
```python
r = httpx.get("http://localhost:8002/api/topics/1")
t = r.json()
...
curl -s -X PUT http://localhost:8002/api/topics/1 \
  -H "Content-Type: application/json" \
...
r = httpx.put("http://localhost:8002/api/topics/1", json={
...
```bash
curl -s -X DELETE http://localhost:8002/api/topics/1
```
```python
r = httpx.delete("http://localhost:8002/api/topics/1")
...
```bash
curl -s -X POST http://localhost:8002/api/topics/1/users/2
```
```python
r = httpx.post("http://localhost:8002/api/topics/1/users/2")
...
```bash
curl -s -X DELETE http://localhost:8002/api/topics/1/users/2
```
```python
r = httpx.delete("http://localhost:8002/api/topics/1/users/2")
print(r.json())
```
...
curl -s -X POST http://localhost:8002/api/searches/run/1 | python3 -m json.tool
```
```python
import httpx

r = httpx.post("http://localhost:8002/api/searches/run/1", timeout=120.0)
...
curl -s "http://localhost:8002/api/searches/runs" | python3 -m json.tool

# Rularile unui topic specific
curl -s "http://localhost:8002/api/searches/runs?topic_id=1"

# Ultimele 10 rulari
curl -s "http://localhost:8002/api/searches/runs?limit=10"

# Combinate
curl -s "http://localhost:8002/api/searches/runs?topic_id=1&limit=5"
```
```python
# Toate rularile unui topic
r = httpx.get("http://localhost:8002/api/searches/runs", params={
    "topic_id": 1,
    "limit": 20
})
for run in r.json():
...
```bash
curl -s http://localhost:8002/api/searches/runs/5 | python3 -m json.tool
```
```python
r = httpx.get("http://localhost:8002/api/searches/runs/5")
run = r.json()
...
curl -s "http://localhost:8002/api/searches/results" | python3 -m json.tool

# Filtrat dupa topic
curl -s "http://localhost:8002/api/searches/results?topic_id=1"

# Cu limita custom
curl -s "http://localhost:8002/api/searches/results?topic_id=1&limit=20"
```
```python
r = httpx.get("http://localhost:8002/api/searches/results", params={
    "topic_id": 1,
    "limit": 50
})
for a in r.json():
...
```bash
curl -s -X DELETE http://localhost:8002/api/searches/results/42
```
```python
r = httpx.delete("http://localhost:8002/api/searches/results/42")
...
```bash
# Deschide in browser
curl -s http://localhost:8002/api/searches/runs/5/preview-email > preview.html && xdg-open preview.html
```
```python
r = httpx.get("http://localhost:8002/api/searches/runs/5/preview-email")
...
curl -s "http://localhost:8002/api/searches/results/export?format=csv" -o articole.csv

# Export JSON filtrat pe un topic
curl -s "http://localhost:8002/api/searches/results/export?format=json&topic_id=1" -o topic1.json

# Export CSV pentru un topic
curl -s "http://localhost:8002/api/searches/results/export?format=csv&topic_id=2" -o topic2.csv
```
```python
# Export CSV in fisier
r = httpx.get("http://localhost:8002/api/searches/results/export",
...
```bash
# Valideaza Anthropic
curl -s http://localhost:8002/api/searches/validate-provider/anthropic | python3 -m json.tool

# Valideaza Tavily
curl -s http://localhost:8002/api/searches/validate-provider/tavily | python3 -m json.tool

# Valideaza SearXNG (self-hosted)
curl -s http://localhost:8002/api/searches/validate-provider/searxng | python3 -m json.tool

# Valideaza Author (OpenAlex + CrossRef)
curl -s http://localhost:8002/api/searches/validate-provider/author | python3 -m json.tool
```
```python
for provider in ["anthropic", "tavily", "searxng", "author"]:
    r = httpx.get(f"http://localhost:8002/api/searches/validate-provider/{provider}",
                  timeout=10.0)
    result = r.json()
